...
service class RequestInterceptor {
    *http:RequestInterceptor;
    resource function 'default [string... path](
        ...
        final readonly & security:AsgardeoJwt|error jwt = security:decodeAsgardeoJwt(xJwtAssertion);

        if jwt is error {
            security:logUserAccessIssue(req.rawPath, jwt);
        ...
        if !utils:isWso2Email(jwt.email) {
            security:logUserAccessIssue(req.rawPath, error(string `Email is not a valid WSO2 email. Email: ${
                jwt.email}`));
            return <types:Forbidden>{
                body: {message: types:NO_PRIVILEGES_ERROR}
            };
...
function checkIfAdminOnlyPath(string[] path, string method) returns boolean {
    if path.length() > 0 {
        ...
        if security:adminPathToAllowedMethods.hasKey(resourcePath) {
            string[] allowedMethods = security:adminPathToAllowedMethods.get(resourcePath);
