...
@display {
    label: "Leave App Backend Service",
    id: "hris-web/leave-app-backend-service"
}
service http:InterceptableService / on new http:Listener(9090) {
    ...
    resource function get form\-data(http:RequestContext ctx) returns types:FormData|types:InternalServerError {
        do {
            security:AsgardeoJwt {email} = check ctx.getWithType(types:JWT_CONTEXT_KEY);
            final readonly & string[] emails = [email];
            future<entity:LeaveEntity[]|error> futureLeaves = start entity:getLeaves({
                emails,
                orderBy: entity:DESC
            }, 'limit = 1);
            future<readonly & entity:EmployeeEntity|error> futureEmployee = start entity:getEmployee(email);
            future<entity:EmployeeEntity[]|error> futureManagedEmployees =
                start entity:getEmployees({managerEmail: email});

            final readonly & entity:EmployeeEntity|error employee = wait futureEmployee;
            if employee is error {
                fail error(types:ERR_MSG_EMPLOYEE_RETRIEVAL_FAILED, employee);
            }

            entity:EmployeeEntity[]|error managedEmployees = wait futureManagedEmployees;
            if managedEmployees is error {
                fail error(types:ERR_MSG_EMPLOYEES_RETRIEVAL_FAILED, managedEmployees);
            }

            entity:LeaveEntity[]|error leaves = wait futureLeaves;
            if leaves is error {
                fail error(types:ERR_MSG_LEAVES_RETRIEVAL_FAILED, leaves);
            }
            ...
            if legallyEntitledLeave is error {
                fail error(types:ERR_MSG_LEGALLY_ENTITLED_LEAVE_RETRIEVAL_FAILED, legallyEntitledLeave);
            }

            return {
                emailRecipients,
                leadEmails,
                isLead: managedEmployees.length() > 0,
                location,
                legallyEntitledLeave,
                leaveReportContent: getLeaveReportContent(leaves)
            };
        } on fail error internalErr {
            log:printError(internalErr.message(), internalErr, stackTrace = internalErr.stackTrace());
    ...
    resource function get leaves(http:RequestContext ctx, string? email = (), string? startDate = (), string? endDate = (), boolean? isActive = ())
        returns types:FetchedLeavesRecord|types:Forbidden|types:InternalServerError {
        final readonly & security:AsgardeoJwt|error decodedJwt = ctx.getWithType(types:JWT_CONTEXT_KEY);
    ...
    resource function post leaves(http:RequestContext ctx, types:LeavePayload payload, boolean isValidationOnlyMode = false)
        returns types:Success|types:CalculatedLeave|types:BadRequest|types:InternalServerError {
        security:AsgardeoJwt|error decodedJwt = ctx.getWithType(types:JWT_CONTEXT_KEY);
        if decodedJwt is error {
        ...
        final readonly & email:EmailNotificationDetails emailContentForLeave = email:generateContentForLeave(email, payload);
        final readonly & string calendarEventId = createUuidForCalendarEvent();
        final readonly & string[] allRecipientsForUser = getAllEmailRecipientsForUser(email, payload.emailRecipients);
        ...
        final readonly & entity:LeaveEntity|error leave = entity:createLeave({...payload}, email);
        if leave is error {
        ...
        future<error?> notificationFuture = start email:sendLeaveNotification(
            emailContentForLeave, allRecipientsForUser);
    ...
    isolated resource function delete leaves/[int leaveId](http:RequestContext ctx)
        ...
        _ = start email:sendLeaveNotification(generateContentForLeave.cloneReadOnly(), allRecipientsForUser.cloneReadOnly());
    ...
    resource function post generate\-report(http:RequestContext ctx, types:ReportPayload payload)
        returns types:ReportContent|types:InternalServerError {
        var {location, businessUnit, department, team, employeeStatuses, startDate, endDate} = payload;
        entity:EmployeeEntity[]|error employees = entity:getEmployees({
            location,
            businessUnit,
            department,
            team,
        ...
        return getLeaveReportContent(entityLeaves);
    }
    ...
    # + payload - Request payload
    # + return - Return leave report on success, otherwise an error response
    resource function post generate\-lead\-report(http:RequestContext ctx, types:LeadReportPayload payload)
        returns types:ReportContent|types:Forbidden|types:InternalServerError {
        security:AsgardeoJwt|error decodedJwt = ctx.getWithType(types:JWT_CONTEXT_KEY);
        if decodedJwt is error {
            log:printError("Error occurred while retrieving JWT from request context", decodedJwt);
            return <types:InternalServerError>{
        ...
        if emails.length() == 0 {
            return <types:Forbidden>{
                body: {
                    message: types:ERR_MSG_FORBIDDEN_TO_NON_LEADS
                }
            };
        }
        entity:LeaveEntity[]|error entityLeaves = entity:getLeaves({emails, isActive: true, startDate, endDate});
    ...
    resource function get report\-filters(string[]? employeeStatuses)
        returns types:ReportFilters|types:BadRequest|types:InternalServerError {
        ...
        if orgData is error {
            log:printError(types:ERR_MSG_ORGANIZATION_DATA_RETRIEVAL_FAILED, orgData);
            return <types:InternalServerError>{
                body: {
    ...
    # + team - Employee team
    # + employeeStatuses - Employee statuses to filter the employees
    # + managerEmail - Manager email to filter the employees
    ...
    resource function get employees(string? location, string? businessUnit, string? department, string? team,
            string[]? employeeStatuses, string? managerEmail)
        returns types:Employee[]|types:InternalServerError {
        entity:EmployeeEntity[]|error employees = entity:getEmployees({
            location,
            businessUnit,
            department,
        ...
        types:Employee[] employeesToReturn = from entity:EmployeeEntity employee in employees
            select {
    ...
    resource function get employees/[string email]() returns types:Employee|types:InternalServerError {
        final entity:EmployeeEntity|error employee = entity:getEmployee(email);
        if employee is error {
    ...
    resource function get user\-calendar(http:RequestContext ctx, string startDate, string endDate)
        returns types:UserCalendarInformation|types:InternalServerError {
        security:AsgardeoJwt|error decodedJwt = ctx.getWithType(types:JWT_CONTEXT_KEY);
        if decodedJwt is error {
            log:printError(types:ERR_MSG_HTTP_CONTEXT_RETRIEVAL_FAILED, decodedJwt);
            return {
    ...
    resource function get employees/[string email]/leave\-entitlement(http:RequestContext ctx, int[]? years = ())
        returns types:LeaveEntitlement[]|types:Forbidden|types:InternalServerError {
        final readonly & security:AsgardeoJwt|error decodedJwt = ctx.getWithType(types:JWT_CONTEXT_KEY);
        if decodedJwt is error {
            log:printError(types:ERR_MSG_HTTP_CONTEXT_RETRIEVAL_FAILED, decodedJwt);
            return <types:InternalServerError>{
        ...
        final readonly & entity:EmployeeEntity|error employee = entity:getEmployee(email);
        if employee is error {
