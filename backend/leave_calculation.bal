...
isolated function createLeaveEventInCalendar(string email, entity:LeaveEntity leave, string calendarEventId) {
    ...
    if location is string && types:timezoneOffsetMap.hasKey(location) {
        timeZoneOffset = types:timezoneOffsetMap.get(location);
    }
...
isolated function getLegallyEntitledLeave(readonly & entity:EmployeeEntity employee) returns types:LeavePolicy|error {
...
isolated function getUserCalendarInformation(string email, string startDate, string endDate)
    ...
    final readonly & entity:EmployeeEntity employee = employeeEntityResponse;
...
isolated function getLeaveEntitlement(readonly & entity:EmployeeEntity employee, int[] years) returns types:LeaveEntitlement[]|error {
...
isolated function getPolicyAdjustedLeaveCounts(readonly & entity:EmployeeEntity employee, int? year = ()) returns types:LeavePolicy|error {
    types:LeavePolicy leavePolicy = check getLegallyEntitledLeave(employee);
