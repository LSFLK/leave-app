      ...
      >
        ...
        >
          <span>
            {filteredLeaves.length !== 0 && employee &&
              <Paper elevation={3} sx={{ width: '100%', p: 1, backgroundColor: theme.palette.grey[200] }}>
                <Typography variant='h6' gutterBottom>Entitled leave taken</Typography>
          ...
          <span>
            {filteredLeaves.length !== 0 && employee &&
              <Paper elevation={3} sx={{ width: '100%', p: 1, backgroundColor: theme.palette.grey[200] }}>
                <Typography variant='h6' gutterBottom>Leave Policy</Typography>
                {isLeaveEntitlementLoading ? <CircularProgress color="secondary" size={16} /> :
                  <Stack
                    direction="row"
                    justifyContent="flex-start"
                  ...
                  >{
                      leaveEntitlement.map(entitlement => (
                        ...
                        >
                          <span>{entitlement.year && <Chip label={`${entitlement.year}`} size="small" />}</span>
                          <span>{entitlement.leavePolicy.casual !== undefined && <Typography variant='caption'>{"Casual"}: {entitlement.leavePolicy.casual}</Typography>}</span>
                          <span>{entitlement.leavePolicy.annual !== undefined && <Typography variant='caption'>{"Annual"}: {entitlement.leavePolicy.annual}</Typography>}</span>
                        </Stack>
                      ))
                    }
                  </Stack>
