...
isolated cache:Cache hrEntityCache = new (defaultMaxAge = 1800, cleanupInterval = 900);
isolated EmployeeEntity[]? lastFetchedEmployees = ();
...
# + return - Return Employee enitity
public isolated function getEmployee(string email) returns EmployeeEntity|error {
    lock {
        any|cache:Error cachedEmployee = hrEntityCache.get(email);
        if cachedEmployee is EmployeeEntity {
            return cachedEmployee.cloneReadOnly();
        }
...
# + email - Employee email
# + return - Return Employee enitity
isolated function getEmployeeFromEntity(string email) returns EmployeeEntity|error {
    string query = string `
    ...
    final readonly & EmployeeEntity employee = response.data.employee.cloneReadOnly();
    lock {
        cache:Error? put = hrEntityCache.put(email, employee);
        if put is cache:Error {
...
#
# + return - Return Employee enitities
public isolated function getAllEmployees()
    returns EmployeeEntity[]|error {
    EmployeeEntity[]? employeesToReturn = ();
    lock {
        if lastFetchedEmployees is EmployeeEntity[] {
            employeesToReturn = lastFetchedEmployees.cloneReadOnly();
        }
    }

    if employeesToReturn is () {
        return getAllEmployeesFromEntity();
    }

    boolean shouldFetch = false;
    lock {
        if !hrEntityCache.hasKey(EMPLOYEE_CACHE_KEY) {
            shouldFetch = true;
        }
    }

    if shouldFetch {
        _ = start getAllEmployeesFromEntity();
    }
...
#
# + return - Return Employee enitities
isolated function getAllEmployeesFromEntity()
    returns EmployeeEntity[]|error {
    ...
    while resultLength == 'limit || firstPage {
        map<anydata> variables = {
            filter: {
                employeeStatus: [EmployeeStatusActive, EmployeeStatusMarkedLeaver]
            },
            'limit,
            offset
        };
    ...
    lock {
        cache:Error? cachingErr = hrEntityCache.put(EMPLOYEE_CACHE_KEY, true);
        if cachingErr is error {
            log:printWarn("Error while caching employees: " + cachingErr.message());
        }
    }

    lock {
        lastFetchedEmployees = queriedEmployees.cloneReadOnly();
    }
...
# + return - Return Employee enitities
public isolated function getEmployees(string? location = (), string? businessUnit = (), string? department = (), string? team = (), string[]? employeeStatuses = ())
    returns EmployeeEntity[]|error {
    final readonly & string[]? employeeStatus = employeeStatuses.cloneReadOnly() ?: [EmployeeStatusActive, EmployeeStatusMarkedLeaver];
    lock {
        any|cache:Error cachedEmployees = hrEntityCache.get(getFetchEmployeesCachingKey(location,
                businessUnit,
                department,
                team,
                employeeStatus));
        if cachedEmployees is EmployeeEntity[] {
            return cachedEmployees.cloneReadOnly();
        }
    }

    return getEmployeesFromEntity(location, businessUnit, department, team, employeeStatus);
}
...
# + employeeStatuses - Employee statuses
# + return - Return Employee enitities
public isolated function getEmployeesFromEntity(string? location = (), string? businessUnit = (), string? department = (), string? team = (), string[]? employeeStatuses = ())
    returns EmployeeEntity[]|error {
    boolean firstPage = true;
    int resultLength = 0;
    int 'limit = MAX_QUERY_PAGE_LIMIT;
    ...
    lock {
        cache:Error? cachingErr = hrEntityCache.put(getFetchEmployeesCachingKey(location,
                businessUnit,
                department,
                team,
                employeeStatus),
...
public isolated function getOrgData(EmployeeStatus[]? employeeStatuses = ()) returns OrgDataEntity|error {
...
#
# + id - Leave ID
# + return - Return Leave enitity
public isolated function getLeave(int id) returns LeaveEntity|error? {
    string query = string `
...
# + return - Return Leave enitities
public isolated function getLeaves(LeaveFilter filter, int? 'limit = (), int? offset = 0) returns LeaveEntity[]|error {
    boolean firstPage = true;
    int resultLength = 0;
...
# + return - Return Leave enitity
public isolated function createLeave(types:LeavePayload payload, string email) returns LeaveEntity|error {
    string query = string `
...
# + email - Email of the user creating the leave
# + return - Return Leave enitity
public isolated function validateLeave(types:LeavePayload payload, string email) returns LeaveEntity|error {
    string query = string `
    ...
    LeaveEntity? validateLeave = response.data?.insertLeave;
    if validateLeave is () {
...
#
# + id - Leave ID
# + return - Return cancelled Leave enitity
...
# + return - Return Office Holidays enitities
public isolated function getHolidays(string country, string startDate, string endDate) returns HolidayEntity[]|error {
    ...
    handleGraphQLResponseError(response.errors, HR_ENTITY_NAME);
    return response.data.officeHolidays;
}
